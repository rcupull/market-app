...
import { useBusinessUpdateSeoModal } from '.';

import { getWrapper } from 'utils/test-utils';

describe('useBusinessUpdateSeoModal', () => {
  it('should close when click on the close button', async () => {
    const { result } = renderHook(() => useBusinessUpdateSeoModal(), {
      wrapper: getWrapper({ useRouter: true, useModal: true }),
    });

    act(() => result.current.businessUpdateSeoModal.open());
      ...
      expect(screen.getByTestId('Modal')).toMatchInlineSnapshot(`
        ...
        >
          ...
          >
            ...
            >
              <div
                class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 w-11/12 md:w-8/12 lg:w-8/12 !w-[95vw] !max-w-[40rem]"
                data-headlessui-state="open"
                data-open=""
