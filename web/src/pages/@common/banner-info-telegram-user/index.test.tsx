import { render } from '@testing-library/react';

import { BannerInfoTelegramUser } from '.';

import { dummyStatus } from 'constants/api';
import { authDataDummy } from 'utils/test-dummies';
import { getWrapper } from 'utils/test-utils';

describe('BannerInfoTelegramUser', () => {
  it('render', async () => {
    const result = render(<BannerInfoTelegramUser />, {
      wrapper: getWrapper({
        useRedux: true,
        useRouter: true,
        initialReduxState: { useAuth: { data: authDataDummy, status: dummyStatus } },
      }),
    });

    expect(result.container).toMatchInlineSnapshot(`
      <div>
        <div
          class="flex items-center ring-1 ring-gray-400 rounded-2xl py-0.5 px-2"
        >
          <span
            class="font-bold"
          >
            <span
              class="text-nowrap"
            >
              Telegram
            </span>
            :
          </span>
          <span>
            <span
              class="text-nowrap"
            >
              <div
                class="flex items-center"
              >
                <svg
                  class="fill-red-500 size-5"
                  height="1em"
                  viewBox="0 0 32 32"
                  width="1em"
                >
                  <path
                    d="M16 3C8.832 3 3 8.832 3 16s5.832 13 13 13 13-5.832 13-13S23.168 3 16 3zm0 2c6.086 0 11 4.914 11 11s-4.914 11-11 11S5 22.086 5 16 9.914 5 16 5zm-3.781 5.781L10.78 12.22 14.562 16l-3.78 3.781 1.437 1.438L16 17.437l3.781 3.782 1.438-1.438L17.437 16l3.782-3.781-1.438-1.438L16 14.562z"
                  />
                </svg>
                <button
                  class="relative px-3 py-1.5 text-sm shadow-sm font-semibold rounded-md flex items-center justify-center leading-6 whitespace-nowrap h-fit text-indigo-600 fill-indigo-600 hover:text-indigo-500 !shadow-none !m-0 !p-0 h-fit"
                >
                  Activar
                </button>
              </div>
            </span>
          </span>
        </div>
      </div>
    `);
  });
});
